      ...
      <div className="container" style={{ maxWidth: '900px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '56px' }}>
          ...
          <h2 style={{
            ...
            marginBottom: '16px',
            fontWeight: 700
          }}>{t("Demande de devis & Réservation")}</h2>
          <p style={{ color: '#666', fontSize: '16px', maxWidth: '560px', margin: '0 auto', lineHeight: 1.6 }}>
            {t("Complétez ce formulaire et cliquez sur le bouton pour démarrer une discussion")} <strong>WhatsApp</strong> {t("avec notre équipe.")}
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid #eee' }}>
            {[
              { val: 'devis', label: `📋 ${t('Demande de devis')}` },
              { val: 'reservation', label: `✈️ ${t('Réservation directe')}` }
            ].map(opt => (
              <button
                key={opt.val}
                type="button"
          ...
          {formStatus === 'success' ? (
            <div style={{ padding: '60px 40px', textAlign: 'center' }}>
              ...
              <p style={{ color: '#666', lineHeight: 1.6, maxWidth: '420px', margin: '0 auto 32px' }}>
                {t("Merci")} <strong>{formData.nom || t('pour votre demande')}</strong>. {t("Notre équipe vous contactera dans les")} <strong>{t("24h")}</strong> {t("pour finaliser votre voyage.")}
              </p>
              <button
              ...
              >{t("Faire une autre demande")}</button>
            </div>
          ) : (
            <form onSubmit={handleFormSubmit} style={{ padding: isMobile ? '32px 20px' : '48px' }}>
              {formStatus === 'error' && (
                <div style={{ backgroundColor: '#fef2f2', border: '1px solid #fecaca', borderRadius: '8px', padding: '14px 20px', marginBottom: '32px', color: '#b91c1c', fontSize: '14px', fontWeight: 600 }}>
                  ⚠️ {t("Une erreur est survenue. Veuillez réessayer.")}
                </div>
              )}

              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
                <InputGroup label={t("Nom complet")} name="nom" value={formData.nom} onChange={handleFormChange} icon={<User size={14} />} placeholder={t("Votre nom et prénom")} required />
                <InputGroup label={t("Email")} name="email" value={formData.email} onChange={handleFormChange} icon={<Mail size={14} />} type="email" placeholder="votre@email.com" required />
                <InputGroup label={t("Téléphone (whatsApp)")} name="telephone" value={formData.telephone} onChange={handleFormChange} icon={<Phone size={14} />} type="tel" placeholder="+33 6 00 00 00 00" />
                <InputGroup label={t("Participants")} name="participants" value={formData.participants} onChange={handleFormChange} icon={<Users size={14} />} type="number" min="1" />
                <InputGroup label={t("Date de départ")} name="dateDepart" value={formData.dateDepart} onChange={handleFormChange} icon={<Calendar size={14} />} type="date" required />
                {formData.typeVoyage === 'devis' && (
                  <InputGroup label={t("Durée (jours)")} name="duree" value={formData.duree} onChange={handleFormChange} icon={<Calendar size={14} />} type="number" placeholder="Ex: 14" />
                )}
              </div>

              <div style={{ marginBottom: '32px' }}>
                <label style={labelStyle}><MessageSquare size={14} /> {t("Message ou précisions")}</label>
                <textarea 
                  name="message" value={formData.message} onChange={handleFormChange}
                  placeholder={t("Avez-vous des attentes particulières ? (hébergement, activités, budget...)")}
                  style={textareaStyle}
                />
              </div>
              ...
              >
                <Send size={18} /> {formData.typeVoyage === 'devis' ? t('Contacter sur WhatsApp') : t('Envoyer')}
              </button>
            </form>
          )}
        </div>

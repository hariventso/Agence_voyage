      ...
      <section style={{
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2, padding: '0 20px' }}>
          ...
          <p style={{ color: '#FFFFFF', fontSize: '1.2rem', lineHeight: 1.6, maxWidth: '800px', margin: '0 auto', fontWeight: 400 }}>
            {t("Découvrez Madagascar à travers des récits authentiques, pour une immersion au cœur de l’âme malgache.")}
          </p>
        </div>
      </section>
            ...
            <div style={{
              ...
              {posts.map(post => (
                <div key={post.id} style={{ backgroundColor: '#111', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column' }}>
                  <img src={getImageUrl(post.image_url)} alt={t(post.title)} style={{ width: '100%', height: '250px', objectFit: 'cover' }} />
                  <div style={{ padding: '30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <span style={{ color: '#FF8C00', fontSize: '0.9rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px' }}>{t(post.category)}</span>
                    <h3 style={{ fontSize: '1.5rem', margin: '15px 0', color: '#fff', fontWeight: 700, lineHeight: 1.3 }}>{t(post.title)}</h3>
                    <p style={{ color: '#aaa', lineHeight: 1.6, marginBottom: '25px', fontSize: '0.95rem', textAlign: 'justify', flex: 1 }}>
                      {post.content?.length > 150 ? t(post.content.substring(0, 150)) + '...' : t(post.content)}
                    </p>
              ...
              {posts.length === 0 && <p style={{ color: '#aaa', gridColumn: '1/-1', textAlign: 'center' }}>{t("Aucun article disponible pour le moment.")}</p>}
            </div>
          )}
        </div>
      </section>

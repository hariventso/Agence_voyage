...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await findCalendarEventById(result.rows[0].id));
  })
);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await findCalendarEventById(req.params.id));
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ sentEvents, reminderLeadHours: config.calendar.reminderLeadHours, smtpConfigured });
  })
);
